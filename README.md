...
## NEW — World homepage (`site/world.html`)

A fresh cinematic homepage experience, built from scratch per the "cinematic digital fragrance house" brief. The existing pages are untouched; `world.html` is the v2 homepage candidate and reuses the approved brand system, `js/data.js` catalogue and `js/main.js` commerce engine (bag drawer, add-to-bag, analytics, mobile menu, newsletter).

- **Run:** same as below (`npx serve site`, open `world.html`). No build step, no new dependencies.
- **Journey:** layered parallax hero (Kurinji dawn, textless master — the 16/17 heroes carry baked-in copy and are not used as canvases) → house statement → Ainthinai index → five pinned full-viewport landscape scenes → featured objects → inline Find Your Virai (3 questions + rationale) → craft/materials → gifting (personal + B2B) → Mullai finale → footer.
- **Scene engine (`js/world.js` + `css/world.css`):** one rAF-throttled scroll handler writes a single `--p` custom property per scene; CSS calc() drives every layer (far landscape, drifting air masses, mist, caption, product chip, next-landscape veil). No scroll-driven JS style spam, no layout shift.
- **Candle ritual:** each scene's product chip begins unlit → wick glow → photographic crossfade to the real "lit" product photograph (the `c` images), with a warm pool and restrained flicker. Once lit, it stays lit.
- **Transitions:** each scene's end veil tints toward the next landscape's tone, so the next world arrives atmospherically rather than sliding in.
- **Palai** renders in light mode (ink text on the pale landscape); all other scenes are dark-mode captions with scrims.
- **Commerce exits everywhere:** every scene carries a grounded product chip (price, Add to Bag, View) per the dual-layer rule; B2B never touches D2C checkout.
- **Motion rules honoured:** nature moves (air, mist, flame, parallax depth); UI rests. `prefers-reduced-motion` gets fully static scenes, pre-lit candles, no pinning tracks. No entrance animations on commerce elements.
- **Analytics:** `landscape_view` (per scene), `collection_view`, `quiz_start/quiz_complete/quiz_restart` join the existing event set on `window.dataLayer`.
